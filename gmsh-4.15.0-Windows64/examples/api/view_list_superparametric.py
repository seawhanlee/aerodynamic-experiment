...
tag = gmsh.view.add(f'Linear field on a quadratic segment')
...
# ... appended with the 2 function values of the defined field
u = [1, 3.5]
segment.extend(u)
...
Phi = [1/2, -1/2,  # shape function at xi = -1:  phi_1 = 1/2*(1 - xi)
       1/2,  1/2]  # shape function at xi =  1:  phi_2 = 1/2*(1 + xi)
# Three monomials (1, xi, xi^2) for the geometrical mapping
P_g = [0, 0, 0,
       1, 0, 0,
       2, 0, 0]
# Geometrical mapping in the parametric space  -1 <= xi <= 1
Phi_g = [0, -1/2, 1/2,  # shape function at xi = -1:  phi_1 = 1/2*(-xi + xi^2)
...
# (currently the error estimator only considers the field - force subdivision with "-1")
